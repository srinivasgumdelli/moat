#!/bin/bash
# jira-proxy-wrapper.sh — proxies all jira commands to host tool proxy

PROXY_URL="http://host.docker.internal:9876"
PROXY_TOKEN=$(cat /etc/tool-proxy-token 2>/dev/null)

if [ -z "$PROXY_TOKEN" ]; then
  echo "[jira-proxy] ERROR: /etc/tool-proxy-token not found" >&2
  exit 1
fi
...
RESPONSE=$(curl -s --max-time 300 -X POST "${PROXY_URL}/jira" \
...
if [ $CURL_EXIT -ne 0 ]; then
  echo "[jira-proxy] ERROR: Tool proxy unreachable (is it running on the host?)" >&2
  exit 1
fi

# Verify response is JSON before parsing
if ! echo "$RESPONSE" | jq -e '.exitCode' >/dev/null 2>&1; then
  echo "[jira-proxy] ERROR: Tool proxy unreachable or returned invalid response" >&2
  exit 1
fi
...
if [ "$BLOCKED" = "true" ]; then
  REASON=$(echo "$RESPONSE" | jq -r '.reason // "blocked by policy"')
  echo "[tool-proxy] BLOCKED: $REASON" >&2
  exit 126
fi
