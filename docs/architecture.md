...
The devcontainer at `~/.devcontainers/moat/` (symlinked to the repo checkout) uses iptables rules to restrict outbound network access. This fails on Apple Silicon because iptables (legacy and nft) doesn't work under Rosetta x86_64 emulation. We're replacing it with Docker network isolation + a squid forward proxy sidecar.
...
All files in the repo checkout (symlinked from `~/.devcontainers/moat/`).
...
Volumes: `moat-bashhistory`, `moat-config` (compose manages naming)
