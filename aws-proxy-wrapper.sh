#!/bin/bash
# aws-proxy-wrapper.sh — proxies all aws CLI commands to host tool proxy
# Read-only mode: mutating commands (create, delete, terminate, etc.) are blocked by the proxy

PROXY_URL="http://host.docker.internal:9876"
PROXY_TOKEN=$(cat /etc/tool-proxy-token 2>/dev/null)

if [ -z "$PROXY_TOKEN" ]; then
  echo "[aws-proxy] ERROR: /etc/tool-proxy-token not found" >&2
  exit 1
fi
...
if [ $CURL_EXIT -ne 0 ]; then
  echo "[aws-proxy] ERROR: Tool proxy unreachable (is it running on the host?)" >&2
  exit 1
fi

# Verify response is JSON before parsing
if ! echo "$RESPONSE" | jq -e '.exitCode' >/dev/null 2>&1; then
  echo "[aws-proxy] ERROR: Tool proxy unreachable or returned invalid response" >&2
  exit 1
fi
...
if [ "$BLOCKED" = "true" ]; then
  ...
  echo "[aws-proxy] BLOCKED: $REASON" >&2
  exit 126
fi
