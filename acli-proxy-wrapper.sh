#!/bin/bash
# acli-proxy-wrapper.sh — proxies all acli commands to host tool proxy

PROXY_URL="http://host.docker.internal:9876"
PROXY_TOKEN=$(cat /etc/tool-proxy-token 2>/dev/null)

if [ -z "$PROXY_TOKEN" ]; then
  echo "[acli-proxy] ERROR: /etc/tool-proxy-token not found" >&2
  exit 1
fi
...
RESPONSE=$(curl -s --max-time 300 -X POST "${PROXY_URL}/acli" \
...
if [ $CURL_EXIT -ne 0 ]; then
  echo "[acli-proxy] ERROR: Tool proxy unreachable (is it running on the host?)" >&2
  exit 1
fi

# Verify response is JSON before parsing
if ! echo "$RESPONSE" | jq -e '.exitCode' >/dev/null 2>&1; then
  echo "[acli-proxy] ERROR: Tool proxy unreachable or returned invalid response" >&2
  exit 1
fi
