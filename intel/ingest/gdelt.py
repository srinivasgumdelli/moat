...
TOPIC_QUERIES = {
    ...
    "geopolitics": (
        "international relations OR geopolitical"
        " OR conflict OR diplomacy"
    ),
    "finance": (
        "financial markets OR central bank"
        " OR economic policy OR stock market"
    ),
}
    ...
    async def fetch(self, topic: str) -> list[Article]:
        ...
        try:
            data = await retry_async(
                self._fetch_api, query, max_articles,
            )
        except Exception:
            logger.exception("GDELT API failed for topic '%s'", topic)
            return []
        ...
        logger.info(
            "GDELT fetched %d articles for topic '%s'",
            len(articles), topic,
        )
        return articles

    @staticmethod
    async def _fetch_api(query: str, max_articles: int) -> dict:
        params = {
            "query": query,
            "mode": "ArtList",
