 ...
 * Returns { name: { url, headers? } } for non-localhost HTTP servers.
 * Localhost servers are excluded (they go through direct URL rewriting).
 */
export function extractHttpMcpServers(mcpServers) {
  ...
  for (const [name, config] of Object.entries(mcpServers)) {
    ...
    try {
      ...
      const entry = { url };
      // Carry over static auth headers from host config
  ...
  for (const [name, config] of Object.entries(mcpServers)) {
    if (config?.type === 'url' || config?.url) {
      ...
      if (proxyToken && proxiedServers?.has(name)) {
        ...
        // Localhost or unknown — just rewrite URLs
