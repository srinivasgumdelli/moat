 ...
 * whose command exists in the container. HTTP servers are always forwarded.
 * Non-fatal on any failure.
 */
export async function copyMcpServers(containerName, mcpServers) {
  ...
  for (const [name, config] of Object.entries(mcpServers)) {
    if (config?.type === 'url' || config?.url) {
      // HTTP/URL servers — always forward with URL rewriting
