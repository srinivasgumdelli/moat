...
export async function copyMcpServers(containerName, mcpServers, options = {}) {
  ...
  try {
    ...
    await runCapture('docker', [
      'exec', containerName,
      'sh', '-c',
      `mkdir -p /home/node/.claude && { [ -f ${claudeJsonPath} ] || printf '%s' '{}' > ${claudeJsonPath}; }`,
    ]);

    // IMPORTANT: use `printf '%s'` not `echo`. /bin/sh in the container is
    // dash, and dash's `echo` interprets backslash escapes (e.g. \n → raw
    // newline) which corrupts the JSON jq just produced.
    await runCapture('docker', [
      'exec', containerName,
      'sh', '-c',
      `tmp=$(jq '${jqExpr.replace(/'/g, "'\\''")}' ${claudeJsonPath}) && printf '%s' "$tmp" > ${claudeJsonPath}`,
    ]);
    ...
    await runCapture('docker', [
      'exec', containerName,
      'sh', '-c',
      `[ -f ${settingsPath} ] && tmp=$(jq 'del(.mcpServers)' ${settingsPath}) && printf '%s' "$tmp" > ${settingsPath} || true`,
    ]);
...
export async function writeContainerSettings(containerName, settings) {
  try {
    ...
    await runCapture('docker', [
      'exec', containerName,
      'sh', '-c',
      `tmp=$(jq '${jqExpr.replace(/'/g, "'\\''")}' /home/node/.claude/settings.json) && printf '%s' "$tmp" > /home/node/.claude/settings.json`,
    ]);
 ...
 * This carries over user-level hooks (e.g. nudge) that live in settings.local.json.
...
export async function copySettingsLocal(containerName) {
  ...
  try {
    ...
    log(`Copied host settings.local.json into container ${DIM}(nudge hooks + local overrides)${RESET}`);
  } catch (e) {
