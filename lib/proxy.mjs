...
export const PROXY_PORT = parseInt(process.env.TOOL_PROXY_PORT || '9876');

export { PROXY_LOG };

/**
 * Check if tool proxy is healthy on the configured port.
 */
export function proxyHealthy() {
  return new Promise((resolve) => {
    const req = http.get(`http://127.0.0.1:${PROXY_PORT}/health`, { timeout: 2000 }, (res) => {
...
export async function stopProxy() {
  ...
  // Kill anything still on the proxy port
  await runCapture('lsof', ['-ti', `:${PROXY_PORT}`], { allowFailure: true })
    .then(r => {
...
export function stopProxySync() {
  ...
  try {
    const pids = execSync(`lsof -ti :${PROXY_PORT}`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();
