...
export async function attach(repoDir, args) {
  ...
  // Derive container name from current workspace
  const workspace = process.env.MOAT_WORKSPACE;
  ...
  if (commandExists('mutagen')) {
    ...
    await teardown(repoDir, wsDataDir, projectName);

    log('Starting container with new mount...');
    await startContainer(attachWorkspace, repoDir, wsDataDir);
