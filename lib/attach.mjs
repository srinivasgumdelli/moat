// attach-dir + detach-dir subcommands

import { resolve, basename, join } from 'node:path';
import { existsSync, statSync, readFileSync, writeFileSync, readdirSync } from 'node:fs';
import { createInterface } from 'node:readline';
import { runCapture, runInherit, commandExists } from './exec.mjs';
import { log, err, BOLD, DIM, YELLOW, CYAN, RESET } from './colors.mjs';
import { findContainer, findMoatContainers, getContainerWorkspace, getExtraMountSources, teardown, startContainer } from './container.mjs';
import { generateExtraDirsYamlForAttach } from './compose.mjs';
import { workspaceId, workspaceDataDir } from './workspace-id.mjs';

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

/**
 * Wait for a mutagen sync session to complete initial sync.
 * If transition problems are detected, pause/resume to recover
 * (stale agent staging directories cause this on reconnect).
 */
async function waitForSync(safeName, { timeoutSec = 120, pollMs = 3000 } = {}) {
  const selector = `moat=true,moat-dir=${safeName}`;
  const deadline = Date.now() + timeoutSec * 1000;
  let retried = false;

  while (Date.now() < deadline) {
    const result = await runCapture('mutagen', [
      'sync', 'list', '--label-selector', selector,
    ], { allowFailure: true });

    const out = result.stdout;

    if (out.includes('Watching for changes')) {
      return; // sync complete
    }

    if (out.includes('Transition problems') && !retried) {
      log(`Sync transition problem detected, reconnecting...`);
      await runCapture('mutagen', ['sync', 'pause', '--label-selector', selector], { allowFailure: true });
      await runCapture('mutagen', ['sync', 'resume', '--label-selector', selector], { allowFailure: true });
      retried = true;
    }

    await sleep(pollMs);
  }

  log(`${YELLOW}Sync still in progress after ${timeoutSec}s — it will continue in the background.${RESET}`);
}

function isDirectory(p) {
  try { return existsSync(p) && statSync(p).isDirectory(); } catch { return false; }
}

function prompt(question) {
  const rl = createInterface({ input: process.stdin, output: process.stdout });
  return new Promise((res) => {
    rl.question(question, (answer) => { rl.close(); res(answer); });
  });
}

export async function attach(repoDir, args, workspace) {
  if (!args[0] || !isDirectory(args[0])) {
    err('Usage: moat attach-dir <directory>');
    process.exit(1);
  }

  const attachDir = resolve(args[0]);
  const attachName = basename(attachDir);
  // Mutagen session names: alphanumeric and hyphens only (no underscores)
  // Mutagen label values: alphanumeric, hyphens, underscores, dots; must start/end alphanumeric
  const safeName = attachName.replace(/[^a-zA-Z0-9-]/g, '-').replace(/^-+|-+$/g, '') || 'dir';

  // Find the running moat container — try exact workspace match first, then auto-detect
  let containerName = await findContainer(workspace);
  if (!containerName) {
    const running = await findMoatContainers();
    if (running.length === 0) {
      err("No running moat container. Start a session first with 'moat'.");
      process.exit(1);
    }
    if (running.length === 1) {
      containerName = running[0].name;
      workspace = running[0].workspace;
    } else {
      log('Multiple moat containers running. Which workspace?');
      for (let i = 0; i < running.length; i++) {
        console.log(`  ${BOLD}${i + 1}${RESET}) ${running[i].workspace}`);
      }
      const answer = await prompt(`\n  ${CYAN}?${RESET} Select workspace ${DIM}[1-${running.length}]${RESET} `);
      const idx = parseInt(answer, 10) - 1;
      if (isNaN(idx) || idx < 0 || idx >= running.length) {
        err('Invalid selection.');
        process.exit(1);
      }
      containerName = running[idx].name;
      workspace = running[idx].workspace;
    }
  }

  const hash = workspaceId(workspace);
  const wsDataDir = workspaceDataDir(hash);

  if (commandExists('mutagen')) {
    // --- Live-sync via Mutagen ---

    // Check for existing session
    const existing = await runCapture('mutagen', [
      'sync', 'list', '--label-selector', `moat=true,moat-dir=${safeName}`
    ], { allowFailure: true });

    if (existing.stdout.includes('Name:')) {
      err(`A sync session for '${attachName}' already exists. Detach it first with:`);
      err(`  moat detach-dir ${args[0]}`);
      process.exit(1);
    }

    // Create target directory inside container (as root, since /extra is at the root)
    await runCapture('docker', ['exec', '-u', 'root', containerName, 'mkdir', '-p', `/extra/${attachName}`]);
    await runCapture('docker', ['exec', '-u', 'root', containerName, 'chown', 'node:node', `/extra/${attachName}`]);

    // Create mutagen sync session
    await runInherit('mutagen', [
      'sync', 'create',
      '--name', `moat-${safeName}`,
      '--label', 'moat=true',
      '--label', `moat-dir=${safeName}`,
      '--sync-mode', 'two-way-resolved',
      '--default-owner-beta', 'node',
      '--default-group-beta', 'node',
      '--ignore-vcs',
      attachDir,
      `docker://${containerName}/extra/${attachName}`,
    ]);

    // Wait for initial sync, pause/resume if transition problems are detected
    await waitForSync(safeName);

    // Update per-workspace path mappings so the proxy can resolve the new /extra/ path
    const mapFile = join(wsDataDir, 'path-mappings.json');
    let mappings = {};
    try {
      if (existsSync(mapFile)) {
        mappings = JSON.parse(readFileSync(mapFile, 'utf-8'));
      }
    } catch {}
    mappings[`/extra/${attachName}`] = attachDir;
    writeFileSync(mapFile, JSON.stringify(mappings) + '\n');

    log(`Attached ${BOLD}${attachDir}${RESET} -> ${BOLD}/extra/${attachName}${RESET} (live-sync)`);
    log('Tell Claude about it:');
    console.log(`  "I have an additional directory at /extra/${attachName}"`);
  } else {
    // --- Fallback: restart container with new bind mount ---

    log(`${YELLOW}mutagen not installed — falling back to container restart.${RESET}`);
    log(`This will ${BOLD}end the current Claude session${RESET}.`);
    log(`For live-sync without restarting: ${DIM}brew install mutagen-io/mutagen/mutagen${RESET}`);
    console.log('');

    const answer = await prompt(`  ${CYAN}?${RESET} Restart container to add ${BOLD}/extra/${attachName}${RESET}? ${DIM}[y/N]${RESET} `);
    if (!/^y(es)?$/i.test(answer)) {
      log('Aborted.');
      return;
    }

    const attachWorkspace = await getContainerWorkspace(containerName);
    const existingSources = await getExtraMountSources(containerName);

    const overrideFile = `${wsDataDir}/docker-compose.extra-dirs.yml`;
    writeFileSync(overrideFile, generateExtraDirsYamlForAttach(existingSources, attachDir, attachName));

    // Recreate container
    log('Stopping container...');
    await teardown(workspace);

    log('Starting container with new mount...');
    await startContainer(attachWorkspace, repoDir, wsDataDir, `moat-${hash}`);

    log(`Container restarted with ${BOLD}/extra/${attachName}${RESET}`);
    log(`Resume your session: ${BOLD}moat --resume${RESET}`);
    log('Tell Claude about it:');
    console.log(`  "I have an additional directory at /extra/${attachName}"`);
  }
}

export async function detach(args) {
  if (!commandExists('mutagen')) {
    err('mutagen is not installed.');
    process.exit(1);
  }

  if (!args[0]) {
    err('Usage: moat detach-dir <dir|--all>');
    process.exit(1);
  }

  if (args[0] === '--all') {
    await runCapture('mutagen', ['sync', 'terminate', '--label-selector', 'moat=true'], { allowFailure: true });
    // Clean up all /extra/ entries from path mappings
    removeExtraPathMappings();
    log('All moat sync sessions terminated.');
  } else {
    const detachName = basename(args[0]);
    const safeDetachName = detachName.replace(/[^a-zA-Z0-9_-]/g, '-').replace(/^[^a-zA-Z0-9]+|[^a-zA-Z0-9]+$/g, '') || 'dir';
    const result = await runCapture('mutagen', [
      'sync', 'terminate', '--label-selector', `moat=true,moat-dir=${safeDetachName}`
    ], { allowFailure: true });

    if (result.exitCode !== 0) {
      err(`No sync session found for '${detachName}'.`);
      process.exit(1);
    }
    // Remove this dir from path mappings
    removeExtraPathMappings(detachName);
    log(`Detached ${detachName}`);
  }
}

/**
 * Remove /extra/<name> entries from all workspace path-mappings.json files.
 * If name is omitted, removes all /extra/ entries.
 */
function removeExtraPathMappings(name) {
  const wsDir = join(process.env.HOME, '.moat', 'data', 'workspaces');
  if (!existsSync(wsDir)) return;

  let entries;
  try {
    entries = readdirSync(wsDir, { withFileTypes: true });
  } catch { return; }

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const mapFile = join(wsDir, entry.name, 'path-mappings.json');
    try {
      if (!existsSync(mapFile)) continue;
      const mappings = JSON.parse(readFileSync(mapFile, 'utf-8'));
      let changed = false;
      for (const key of Object.keys(mappings)) {
        if (name) {
          if (key === `/extra/${name}`) {
            delete mappings[key];
            changed = true;
          }
        } else {
          if (key.startsWith('/extra/')) {
            delete mappings[key];
            changed = true;
          }
        }
      }
      if (changed) {
        writeFileSync(mapFile, JSON.stringify(mappings) + '\n');
      }
    } catch {}
  }
}
