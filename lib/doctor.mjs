...
import { proxyHealthy, PROXY_PORT } from './proxy.mjs';
...
export async function doctor(repoDir, dataDir) {
  ...
  if (await proxyHealthy()) {
    info(`Tool proxy responding on :${PROXY_PORT}`);
  } else {
    info(`Tool proxy not running on :${PROXY_PORT} (normal outside sessions)`);
  }
  ...
  if (commandExists('mutagen')) {
    pass("mutagen installed (enables 'moat attach-dir')");
    ...
    info("mutagen not installed (optional, for 'moat attach-dir' live-sync)");
  }
