...
const BIN_FILES = [
  ...
  { src: 'quality-gate.sh', dest: '/usr/local/bin/quality-gate' },
];

export async function refreshHooks(containerName, repoDir) {
  // Ensure the persistent volume is user-writable — stale volumes from older
  // image builds can have files with restrictive permissions that prevent
  // Claude Code's worktree isolation from creating sub-directories.
  try {
    await runCapture('docker', [
      'exec', containerName,
      'chmod', '-R', 'u+rw', '/home/node/.claude',
    ]);
  } catch {
    // Non-fatal — best-effort fix
  }
