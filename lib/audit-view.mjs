...
function formatEvent(event) {
  ...
  switch (type) {
    case 'session.start': {
      const sha = event.head_sha ? ` head=${event.head_sha.slice(0, 10)}` : '';
      return `${time}  ${type}  workspace=${event.workspace || '?'} runtime=${event.runtime || 'claude'} version=${event.moat_version || '?'}${sha}`;
    }
    case 'session.end': {
      const sha = event.head_sha ? ` head=${event.head_sha.slice(0, 10)}` : '';
      return `${time}  ${type}  exit_code=${event.exit_code} duration=${event.duration_ms}ms${sha}`;
    }
    case 'tool.execute': {
    ...
    case 'quality_gate.result': {
      const p = event.payload || event;
      const counts = `passed=${p.checks_passed || 0} failed=${p.checks_failed || 0} skipped=${p.checks_skipped || 0}`;
      return `${time}  ${type}  outcome=${p.outcome || '?'} ${counts}`;
    }
    case 'quality_gate.skipped': {
      const p = event.payload || event;
      return `${time}  ${type}  reason=${p.reason || '?'}`;
    }
    case 'checkpoint': {
      return `${time}  ${type}  message=${event.message || '?'}`;
    }
    default:
