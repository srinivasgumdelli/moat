...
export async function copySkills(containerName) {
  ...
  try {
    ...
    // Copy each entry individually to avoid docker cp quirks with trailing /.
    for (const entry of skills) {
      const src = join(HOST_SKILLS_DIR, entry.name);
      const dest = `${containerName}:${CONTAINER_SKILLS_DIR}/${entry.name}`;
      if (entry.isDirectory()) {
        // Ensure subdirectory exists in container first
        await runCapture('docker', [
          'exec', containerName,
          'mkdir', '-p', `${CONTAINER_SKILLS_DIR}/${entry.name}`,
        ]);
        await runCapture('docker', ['cp', `${src}/.`, `${containerName}:${CONTAINER_SKILLS_DIR}/${entry.name}/`]);
      } else {
  ...
  try {
    ...
    for (const entry of commands) {
      ...
      if (entry.isDirectory()) {
        ...
        await runCapture('docker', ['cp', `${src}/.`, `${containerName}:${CONTAINER_COMMANDS_DIR}/${entry.name}/`]);
      } else {
        await runCapture('docker', ['cp', src, dest]);
      }
    }
