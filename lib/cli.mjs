 ...
 *
 * Subcommands: doctor, update, down, stop, attach-dir, detach-dir, uninstall, init
...
export function parseArgs(argv) {
  ...
  const rawSubcommands = new Set([
    ...
    'ps', 'log', 'audit', 'rewind', 'help', 'dispatch', 'sync-skills',
  ]);
  ...
  // Treat --help / -h anywhere as the help subcommand
  if (rest.includes('--help') || rest.includes('-h')) {
    return { subcommand: 'help', subcommandArgs: [], workspace: process.cwd(), extraDirs: [], claudeArgs: [] };
  }
  ...
  // Parse --add-dir, --runtime, --mcp-rw flags and collect remaining claude args
  const extraDirs = [];
  const claudeArgs = [];
  let runtimeArg = null;
  let mcpRw = false;
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    } else if (rest[i] === '--runtime') {
      i++;
      if (i < rest.length) {
        runtimeArg = rest[i];
      } else {
        throw new Error('--runtime requires a runtime name (claude, codex, opencode, amp)');
      }
    } else if (rest[i] === '--mcp-rw') {
      mcpRw = true;
    } else {
  ...
  return { subcommand, subcommandArgs: [], workspace, extraDirs, claudeArgs, runtimeArg, mcpRw };
}
