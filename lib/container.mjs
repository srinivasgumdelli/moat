 ...
 * Files are referenced by absolute path so they can live in wsDataDir.
 */
export function composeArgs(repoDir, wsDataDir, projectName) {
  return [
    '--project-name', projectName,
    '-f', `${repoDir}/docker-compose.yml`,
    '-f', `${wsDataDir}/docker-compose.services.yml`,
    '-f', `${wsDataDir}/docker-compose.extra-dirs.yml`,
  ];
}
 ...
 */
export async function containerRunning(repoDir, workspace, wsDataDir, projectName, containerName) {
  try {
    const ps = await runCapture('docker', ['compose', ...composeArgs(repoDir, wsDataDir, projectName), 'ps', '--status', 'running'], { allowFailure: true });
...
/**
 * Tear down containers for a specific project.
 */
export async function teardown(repoDir, wsDataDir, projectName) {
  await runCapture('docker', ['compose', ...composeArgs(repoDir, wsDataDir, projectName), 'down'], { allowFailure: true });
}
