...
import { copyPlugins } from './plugins.mjs';
import { copyStatusline } from './statusline.mjs';
...
export async function copyInstructions(containerName, repoDir, runtime, workspace) {
  if (runtime.configDir === '.claude') {
    ...
    // Copy installed plugins (commands/skills/hooks/agents/MCPs) from host
    await copyPlugins(containerName, workspace);
    // Copy statusline scripts from host so the forwarded settings resolve
    await copyStatusline(containerName);
