...
function generateComposeYaml(services, envVars, squidRuntimePath) {
  ...
  for (const name of serviceNames) {
    ...
    if (defaults.healthcheck) {
      ...
      const testItems = hc.test.map(t => JSON.stringify(t)).join(', ');
      lines.push(`      test: [${testItems}]`);
  ...
  // Always mount the runtime squid config. Without this override, MCP domains
  // and .moat.yml extra domains silently fall back to the base allowlist.
  lines.push('  squid:');
  lines.push('    volumes:');
  ...
  if (healthyServices.length > 0 || hasEnv) {
 ...
 * seccomp (for user namespace creation). Podman runs rootless inside
...
/**
 * Generate docker-compose.extra-dirs.yml for attach-dir fallback (existing mounts + new).
 */
...
export function generateProjectConfig(workspace, repoDir, wsDataDir, mcpDomains = []) {
  ...
  const extraDomainsPath = join(wsDataDir, 'extra-domains.txt');
  ...
  // Load persisted additions from `moat allow-domain` so they survive regeneration.
  let persistedDomains = [];
  if (existsSync(extraDomainsPath)) {
    try {
      persistedDomains = readFileSync(extraDomainsPath, 'utf8')
        .split('\n')
  ...
  // Generate squid-runtime.conf (add Docker + MCP + persisted domains as needed)
  const seen = new Set();
  const allDomains = [...domains, ...mcpDomains, ...(hasDocker ? DOCKER_DOMAINS : []), ...persistedDomains]
